  ...
  async run(bot, message, args) {
    ...
    JSON.parse(_jsonString).Whitelist.forEach(async (spot) => {
      if (member.id === spot.id) {
        ...
        embed.addField(
          "**Data from User**",
          `**User:** <@${member.id}> **Whitelist Data:** **Wallet Holder = __${_jsonString.Whitelist[i-1].wallet}__**\n**Alpha Holder = __${_jsonString.Whitelist[i-1].alpha}__** - **Is Whitelisted = __${_jsonString.Whitelist[i-1].whitelist}__**\n**The user Upvote in ME = __${_jsonString.Whitelist[i-1].upvote}__**`
        );
